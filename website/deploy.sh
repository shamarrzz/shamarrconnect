...
ACCT_FILE="$HOME/sc-website/.cf-account-id"
...
if [ ! -s "$ACCT_FILE" ]; then
  echo "ERROR: put the Cloudflare account ID in $ACCT_FILE (chmod 600) first." >&2
  exit 1
fi
...
export CLOUDFLARE_ACCOUNT_ID="$(cat "$ACCT_FILE")"
